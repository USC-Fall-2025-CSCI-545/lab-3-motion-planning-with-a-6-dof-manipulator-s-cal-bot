...
class AdaRRT():
    ...
    def build(self):
        ...
        for k in range(self.max_iter):
            ...
            if new_node and self._check_for_completion(new_node):
                ...
                print("Found path from {0} to {1} in {2} iterations!".format(
                    self.start.state, self.goal.state, k + 1))
    ...
    def _get_random_sample_near_goal(self):
        """
        Samples a point near the goal within +/- 0.05 on each joint axis.

        :returns: A vector representing a randomly sampled point near the goal.
        """
        low = self.goal.state - 0.05
        high = self.goal.state + 0.05
    ...
    def _extend_sample(self, sample, neighbor):
        ...
        new_state = neighbor.state + step * direction_unit
    ...
    def _check_for_completion(self, node):
        ...
        if dist_to_goal <= self.goal_precision:
            self.goal.parent = node
            node.children.append(self.goal)
            return True
        return False
